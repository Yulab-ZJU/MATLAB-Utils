function [granger, coh] = mu_granger(trialsDataArea1, trialsDataArea2, windowData, fs, varargin)
...
data.fsample = fs;
data.label = mat2cell(char([mu.rowfun(@(x) strcat('Area1-', strrep(x, ' ', '0')), string(num2str((1:size(trialsDataArea1{1}, 1))'))); ...
                            mu.rowfun(@(x) strcat('Area2-', strrep(x, ' ', '0')), string(num2str((1:size(trialsDataArea2{1}, 1))')))]), ...
