...
for pIndex = 1:numel(rhdPATHs)
    ...
    for fIndex = 1:numel(filenames)
        ...
        if (data_present)
            ...
            t_amplifier{fIndex} = zeros(1, num_amplifier_samples);

            board_dig_in_data{fIndex} = zeros(num_board_dig_in_channels, num_board_dig_in_samples);
            board_dig_in_raw{fIndex}  = zeros(1, num_board_dig_in_samples);
            ...
            for i=1:num_data_blocks
                ...
                if ((data_file_main_version_number == 1 && data_file_secondary_version_number >= 2) ...
                        || (data_file_main_version_number > 1))
                    t_amplifier{fIndex}(amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = fread(fid, num_samples_per_data_block, 'int32');
                else
                    t_amplifier{fIndex}(amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = fread(fid, num_samples_per_data_block, 'uint32');
                end
                if (num_amplifier_channels > 0)
                    dataBuffer = fread(fid, [num_samples_per_data_block, num_amplifier_channels], 'uint16')';
                    dataBuffer = 0.195 * (dataBuffer - 32768); % units = microvolts
                    dataBuffer = dataBuffer(:); % vectorize
                    switch Format
                        case 'i16'
                            fwrite(fidOut, int16(dataBuffer), 'integer*2');
                        case 'f32'
                            fwrite(fidOut, single(dataBuffer), 'single');
                    end
                end
                ...
                if (num_board_dig_in_channels > 0)
                    board_dig_in_raw{fIndex}(board_dig_in_index:(board_dig_in_index + num_samples_per_data_block - 1)) = fread(fid, num_samples_per_data_block, 'uint16');
                end
                if (num_board_dig_out_channels > 0)
    ...
    if data_present
        ...
        % Save trigger
        TTL = board_dig_in_data;
        save(triggers{pIndex}, "TTL", "t_amplifier", "-v7.3");
    end
