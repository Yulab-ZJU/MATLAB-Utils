function [BINPATHs, TRIGPATHs, nch] = mu_ks_exportBins(EXCELPATH, sortIDs, FORMAT, skipBinExportExisted)
% Read parameters from Excel file
sortIDs = unique(sortIDs);
params = mu_ks_getParamsExcel(EXCELPATH, sortIDs);
nBatch = numel(params);
...
for rIndex = 1:nBatch
    ...
    switch lower(recTech)
        case 'tdt'
            [BINPATHs{rIndex}, nch] = mu_ks_getBins_TDT(BLOCKPATHs, "Format", FORMAT, "SkipExisted", skipBinExportExisted);
            TRIGPATHs{rIndex} = BLOCKPATHs;
        case 'rhd'
            [BINPATHs{rIndex}, TRIGPATHs{rIndex}] = mu_ks_getBins_RHD(DATAPATHs, "Format", FORMAT, "SkipExisted", skipBinExportExisted);
            nch = 128;
        case {'neuropixel', 'neuropixels', 'np'}
            [BINPATHs{rIndex}, TRIGPATHs{rIndex}] = mu_ks_getBins_NP(DATAPATHs, skipBinExportExisted);
            nch = 385;
        otherwise
