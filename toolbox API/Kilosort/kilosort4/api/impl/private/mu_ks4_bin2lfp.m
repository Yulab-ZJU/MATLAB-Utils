...
%% Design filters
% Band-pass filter
d_bp = designfilt('bandpassiir', ...
    'FilterOrder', 4, ...
    'HalfPowerFrequency1', opt.fhp, ...
    'HalfPowerFrequency2', opt.flp, ...
    'SampleRate', fs);

assert(opt.fsLFP < fs, 'LFP sample rate must be lower than raw sampling rate %.2f Hz', fs);
...
if strcmp(method,'memmap')
    ...
    % Resample
    if decimFactor > 1
        lfp = mu.resampledata(filt', fs, opt.fsLFP)';  % multi-channel safe
    else
        lfp = filt;
    end
...
while nRemaining > 0
    ...
    % Filtering
    filtChunk = filtfilt(d_bp, rawChunk')';

    % Resample
    if decimFactor > 1
        filtChunk = mu.resampledata(filtChunk', fs, opt.fsLFP)';  % multi-channel safe
    end
